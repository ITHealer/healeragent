...
class FMPStockNewsItem(BaseModel):
    ...
    published_date: datetime = Field(..., alias="publishedDate", description="Publication datetime")
    ...
    def parse_datetime(cls, v):
        """Parse datetime from FMP format"""
        if isinstance(v, datetime):
            return v
        if isinstance(v, str):
            # FMP format: "2024-02-28 14:30:00"
...
class FMPArticleItem(BaseModel):
    """
    FMP Articles from /stable/fmp-articles
    Articles written by FMP analysts
    """
    published_date: datetime = Field(..., alias="publishedDate")
    title: str = Field(...)
    image: Optional[str] = None
