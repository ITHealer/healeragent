...
只返回翻译结果，不要添加任何说明。"""
    ...
    async def _translate_with_chunks(self, text: str, target_lang_name: str, source_lang_name: str) -> str:
        ...
        for i, chunk in enumerate(chunks):
            ...
            system_prompt = f"""You are a professional translator. Please accurately translate the {source_lang_name} text into {target_lang_name}.

This is part {i+1} of the complete document, totaling {len(chunks)} parts.

Translation Requirements:
- Maintain the original format and structure
- Accurately convey the original meaning, using natural and fluent language
- Maintain accurate terminology
- Do not add explanations or annotations
- Maintain coherence with the surrounding context"""

            user_prompt = f"""Please translate the following {source_lang_name} text into {target_lang_name}:

{chunk}

Only return translation results. """
    ...
    def should_translate(self, source_language: str, target_language: str) -> bool:
        ...
        # If the language is the same, no translation is required
        if source_lang == target_lang:
            return False
        
        # Handling special cases for Chinese
