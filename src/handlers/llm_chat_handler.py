...
class ChatHandler(LoggerMixin):
    ...
    #                     max_short_term=5,  # Recent conversations
    #                     max_long_term=3,    # Important historical context
    ...
    #         # Step 4: Combine memory context with retrieval context
    #         combined_context = ""
            
    #         # Combine memory context and document context
    #         if memory_context:
    ...
    async def handle_chat_provider_reasoning_reply_text_stream(
        ...
        Args:
            session_id: Chat session ID
            question_input: User question
            system_language: Target language for response
            model_name: LLM model name
            collection_name: Collection name in vector database
            provider_type: Provider type (ollama, openai, gemini)
            user_id: User ID
            organization_id: Organization ID
            use_multi_collection: Whether to use multiple collections
            clean_thinking: Strip <thinking> contents from final output
            enable_thinking: Allow reasoning tokens during generation
            use_memory_context: Include retrieved memory into context
            reply_to_text: Optional quoted text that user is replying to.
                        If provided, will be merged with question_input.
                        If None/empty, standard flow without reply context.
        ...
        try:
            ...
            # ALWAYS use original question_input for language detection
            # (reply_to_text might be in different language)
            ...
            if session_id and user_id and use_memory_context:
                try:
                    ...
                    # Store in memory
                    # For reply conversations: store merged query to preserve full context
                    # For normal conversations: store original question
    ...
    async def handle_chat_provider_reasoning_stream(
        ...
        try:
            ...
            # Step 2: Save the user's question to the database
            question_id = chat_service.save_user_question(
                session_id=session_id,
                created_at=datetime.datetime.now(),
                created_by=user_id if user_id else "user",
            ...
            # Step 5: Analyze conversation importance
            importance_score = 0.5  # Default score
            
            if session_id and user_id and use_memory_context:
                try:
                    # Determine model for importance analysis
                    analysis_model = "gpt-4.1-nano"
                    ...
                    # Boost importance for certain types of conversations
                    if enable_thinking:  # Complex reasoning conversations are more important
                        importance_score = min(1.0, importance_score + 0.1)
                    
                    if len(complete_response) > 2000:  # Long detailed responses
                        importance_score = min(1.0, importance_score + 0.1)
            ...
            # Step 5: Store conversation in memory system
            if session_id and user_id and use_memory_context:
                try:
                    # Prepare metadata
                    metadata = {
            ...
            # Yield completion signal with memory stats
            # yield f"\n\n[Memory Stats: {json.dumps(memory_stats)}]"
