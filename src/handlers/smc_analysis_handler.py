...
#         """
#         self.logger.info(f"[SMC] Starting analysis for {request.symbol} @ {request.interval}")
...
#         self.logger.info(f"[SMC] Analysis complete: {trading_plan.recommended_action} with {analysis_confidence} confidence")
...
#                 stop_loss = ob_analysis.strongest_demand_zone["low"] * 0.995
#                 sl_reasoning = f"Stop loss below demand zone"
#             elif liquidity.nearest_eql:
#                 stop_loss = liquidity.nearest_eql * 0.995
#                 sl_reasoning = f"Stop loss below EQL"
#             else:
#                 stop_loss = current_price * 0.97
#                 sl_reasoning = "Default stop loss 3% below current price"
#         elif trend.direction == "bearish":
#             if ob_analysis.strongest_supply_zone:
#                 stop_loss = ob_analysis.strongest_supply_zone["high"] * 1.005
#                 sl_reasoning = f"Stop loss above supply zone"
#             elif liquidity.nearest_eqh:
#                 stop_loss = liquidity.nearest_eqh * 1.005
#                 sl_reasoning = f"Stop loss above EQH"
#             else:
#                 stop_loss = current_price * 1.03
#                 sl_reasoning = "Default stop loss 3% above current price"
#         else:
#             stop_loss = current_price * 0.97
#             sl_reasoning = "Neutral trend - conservative stop loss"
...
#                 reasoning="EQH liquidity target"
#             ))
#         elif trend.direction == "bearish" and liquidity.nearest_eql:
#             targets.append(TargetLevelResult(
#                 price=liquidity.nearest_eql,
#                 target_type="liquidity_sweep",
#                 reasoning="EQL liquidity target"
#             ))
        
#         if not targets:
#             target_pct = 1.03 if trend.direction == "bullish" else 0.97
#             targets.append(TargetLevelResult(
#                 price=current_price * target_pct,
#                 target_type="percentage",
#                 reasoning="Default 3% target"
#             ))
...
#         if rr_ratio < 1.5:
#             warnings.append(f"Risk/Reward ratio {rr_ratio:.2f} below optimal 1.5:1 minimum")
#         if trend.last_structure_event == "CHoCH":
#             warnings.append("Recent CHoCH indicates potential trend reversal - use caution")
        
#         invalidation = stop_loss * 0.99 if trend.direction == "bullish" else stop_loss * 1.01
...
#         if metadata.totalStructureEvents >= 5:
#             score += 30
#         elif metadata.totalStructureEvents >= 3:
#             score += 20
#         elif metadata.totalStructureEvents >= 1:
#             score += 10
        
#         if metadata.activeOrderBlocks >= 3:
#             score += 25
#         elif metadata.activeOrderBlocks >= 2:
#             score += 15
#         elif metadata.activeOrderBlocks >= 1:
#             score += 10
        
#         if metadata.totalLiquidityZones >= 4:
#             score += 20
#         elif metadata.totalLiquidityZones >= 2:
#             score += 15
#         elif metadata.totalLiquidityZones >= 1:
#             score += 10
        
#         if metadata.activeFvgCount >= 3:
#             score += 15
#         elif metadata.activeFvgCount >= 2:
#             score += 10
#         elif metadata.activeFvgCount >= 1:
#             score += 5
        
#         if metadata.currentTrend != "neutral":
#             score += 10
...
#         summary = (
#             f"{symbol} {interval}: Trend is {trend.direction} ({trend.strength} strength) "
...
#             summary += f" Primary entry: {first.price_range['low']:.2f}-{first.price_range['high']:.2f}."
