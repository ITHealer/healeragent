...
class SearchRetrieval(LoggerMixin):
    ...
    async def memory_retrieval(
        ...
        """
        Special retrieval method for memory collections without query_headers step
        
        Args:
            query (str): Query string
            top_k (int): Number of top results to return
            collection_name (str): Name of the memory collection to search
        ...
        try:
            ...
            if not docs:
                self.logger.info("[MEMORY RETRIEVE] - No documents found")
                return []
            
            for idx, doc in enumerate(docs[:5]):
                self.logger.info(f"[MEMORY RETRIEVE] Top {idx+1}: type={doc.metadata.get('type', 'N/A')}, "
                                f"timestamp={doc.metadata.get('timestamp', 'N/A')}, "
                                f"content_preview={doc.page_content[:100]}...")
            
            # Rerank documents with lower threshold for memory
            self.logger.info(f"[MEMORY RERANK] - STARTING RERANKING OF {len(docs)} DOCUMENTS - threshold 0.3")
            docs = self._query_retrieval_reranking(docs, query, 0.3)
            
            for idx, doc in enumerate(docs[:5]): 
                self.logger.info(f"[MEMORY RERANK] Top {idx+1}: "
                                f"importance={doc.metadata.get('importance_score', 0)}, "
                                f"content_preview={doc.page_content[:100]}...")
            
            # For memory, we already have full content, no need for query_headers
            self.logger.info(f"[MEMORY RETRIEVE] Retrieved {len(docs)} memory documents")
            
            return docs[:top_k]
            
        except Exception as e:
            self.logger.error(f'[MEMORY RETRIEVE] Failed to retrieve memory: {e}')
            return []
