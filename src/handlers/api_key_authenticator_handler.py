...
from src.database.repository.api_key import APIKeyRepository
...
from src.helpers.crypto_helper import crypto_helper
...
class APIKeyAuth(LoggerMixin):
    ...
    async def get_current_auth_for_ws(
        self,
        websocket: WebSocket,
        auth_payload: str = Query(..., description="API key đã được mã hóa và encode base64."),
        organization_id: Optional[str] = Query(None, alias="X-Organization-Id"),
    ) -> Dict[str, Any]:
        """
        Dependency xác thực cho WebSocket, xử lý payload đã được mã hóa.
        Đã thêm xử lý lỗi chung để tránh lỗi 500.
        """
        decrypted_api_key = None
        try:
            # Bước 1: Giải mã payload
            encrypted_json_str = base64.b64decode(auth_payload).decode('utf-8')
            decrypted_api_key = crypto_helper.decrypt(encrypted_json_str)

            if not decrypted_api_key:
                raise WebSocketException(code=status.WS_1008_POLICY_VIOLATION, reason="Auth payload không hợp lệ.")

            # Bước 2: Dùng key đã giải mã để xác thực
            api_key_data = await self.author_with_api_key_or_bearer(
                organization_id=organization_id,
                api_key=None,
            ...
            self.logger.warning(f"Xác thực WebSocket thất bại (HTTPException): {e.detail}")
            raise WebSocketException(
                code=status.WS_1008_POLICY_VIOLATION,
                reason=e.detail
            )
        except Exception as e:
            self.logger.error(f"Lỗi không mong muốn trong quá trình xác thực WebSocket: {e}", exc_info=True)
            raise WebSocketException(
                code=status.WS_1011_INTERNAL_ERROR,
                reason="Lỗi xác thực nội bộ."
            )