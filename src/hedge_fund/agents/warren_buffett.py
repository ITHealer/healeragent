...
from src.hedge_fund.tools.api_fmp import get_financial_metrics, get_market_cap, search_line_items, LineItemResponse  
...
from src.hedge_fund.llm.models import ModelProvider, get_model
from src.hedge_fund.utils.progress import progress
...
def warren_buffett_agent(state: AgentState, agent_id: str = "warren_buffett_agent"):
    ...
    for ticker in tickers:
        ...
        if hasattr(line_items_data, 'search_results'):
            financial_line_items = line_items_data.search_results
        else:
            # Handle original list format
            financial_line_items = line_items_data if isinstance(line_items_data, list) else []

        progress.update_status(agent_id, ticker, "Getting market cap")

        # Get current market cap
        market_cap_data = get_market_cap(ticker, end_date, api_key=api_key)

        # Extract market cap value based on return type
        if hasattr(market_cap_data, 'company_facts'):
            # It's a CompanyFactsResponse from api_fmp.py
            market_cap = market_cap_data.company_facts.market_cap if market_cap_data.company_facts else None
        elif isinstance(market_cap_data, (int, float)):
            # It's a direct numeric value from original API
            market_cap = float(market_cap_data)
        else:
            market_cap = None
...
def generate_buffett_output(
    ...
    for key, value in analysis_data.items():
        if hasattr(value, 'model_dump'):
            ...
            serializable_data[key] = [item.model_dump() for item in value]
        else:
            # Regular data
            serializable_data[key] = value

    prompt = template.invoke({"analysis_data": json.dumps(serializable_data, indent=2, default=str), "ticker": ticker})
...
class WarrenBuffettChatAgent:
    """Warren Buffett agent với FMP data integration từ src xyz"""
    
    def __init__(self):
        self.name = "Warren Buffett"
        self.agent_id = "warren_buffett"
        
        # Initialize FMP services từ src xyz
        self.tool_call_service = ToolCallService()
        self.equity_detail_service = EquityDetailService()
        ...
        logger.info(f"Initialized {self.name} agent with FMP integration")
        logger.info(f"FMP API configured: {bool(self.fmp_api_key and self.fmp_api_key != 'YOUR_FMP_API_KEY_PLACEHOLDER')}")
    ...
    async def chat_with_analysis(
        ...
        """Chat với phân tích real-time từ FMP"""
        
        logger.info(f"=== WARREN BUFFETT FMP ANALYSIS START ===")
        ...
        if not self.fmp_api_key or self.fmp_api_key == "YOUR_FMP_API_KEY_PLACEHOLDER":
            ...
            return {
                "content": "I cannot access market data as the FMP API is not configured. Let me share some timeless investment wisdom instead...",
                "tickers_analyzed": [],
                "analysis_data": {},
                "sources": []
            }
        ...
        if not tickers:
            ...
            return await self.chat_general(
                message=message,
                context=context,
        ...
        # Analyze each ticker using FMP
        for ticker in tickers[:3]:
            logger.info(f"\n--- Analyzing {ticker} with FMP ---")
            try:
                analysis = await self.analyze_stock_fmp(ticker)
                
                if analysis:
                    analysis_results[ticker] = analysis
                    logger.info(f"✅ {ticker} FMP analysis complete")
                    logger.info(f"  Signal: {analysis.get('signal')}")
                    logger.info(f"  MOS: {analysis.get('margin_of_safety')}%")
                else:
                    logger.error(f"❌ {ticker} analysis returned None")
        ...
        if not analysis_results:
            ...
            return {
                "content": "Market data is temporarily unavailable. Let me share some investment principles instead...",
                "tickers_analyzed": [],
                "analysis_data": {},
                "sources": []
            }
        ...
        # Format analysis
        market_context = self._format_fmp_analysis(analysis_results)
        logger.info(f"Formatted analysis: {len(market_context)} chars")
        
        # Generate response
        system_prompt = f"""{personality}

You have analyzed these companies using REAL FMP financial data:

{market_context}

INSTRUCTIONS:
1. Reference the ACTUAL numbers above
2. Discuss the specific financial metrics
3. Explain the margin of safety calculation
4. Give clear investment recommendation
5. Use your folksy wisdom to interpret the data"""
        ...
        from src.hedge_fund.llm.models import get_model, normalize_provider_name
        
        if isinstance(provider_type, str):
            model_provider = normalize_provider_name(provider_type)
        else:
        ...
        return {
            ...
            "tickers_analyzed": list(analysis_results.keys()),
            "analysis_data": analysis_results,
            "sources": [f"FMP Financial Data for {ticker}" for ticker in analysis_results.keys()]
        }
    
    async def analyze_stock_fmp(self, ticker: str) -> Dict[str, Any]:
        """
        Phân tích stock sử dụng FMP API từ src xyz services
        """
        logger.info(f"\n{'='*50}")
        logger.info(f"Starting FMP analysis for {ticker}")
        logger.info(f"{'='*50}")
        
        try:
            # 1. GET KEY METRICS TTM
            logger.info(f"[1/7] Fetching key metrics TTM...")
            progress.update_status("warren_buffett_agent", ticker, "Fetching metrics")
            
            key_metrics = await self.tool_call_service.get_key_metrics_ttm(symbol=ticker)
            
            if not key_metrics:
                logger.error(f"No key metrics for {ticker}")
                raise ValueError(f"No key metrics for {ticker}")
            
            logger.info(f"✅ Key metrics:")
            logger.info(f"  - P/E: {key_metrics.peRatioTTM}")
            logger.info(f"  - ROE: {key_metrics.roeTTM}")
            logger.info(f"  - Market Cap: ${key_metrics.marketCapTTM:,.0f}")
            
            # 2. GET FINANCIAL STATEMENTS (using get_financial_statements)
            logger.info(f"[2/7] Fetching financial statements...")
            progress.update_status("warren_buffett_agent", ticker, "Fetching statements")
            
            async with httpx.AsyncClient() as client:
                # Get income statement
                income_data = await self.equity_detail_service.get_income_statement(
                    symbol=ticker,
                    period="annual",
                    limit=5,
                    client=client
                ...
                # Get balance sheet
                balance_data = await self.equity_detail_service.get_balance_sheet_statement(
                    symbol=ticker,
                    period="annual",
                    limit=5,
                    client=client
                ...
                # Get cash flow
                cashflow_data = await self.equity_detail_service.get_cash_flow_statement(
                    symbol=ticker,
                    period="annual",
                    limit=5,
                    client=client
            ...
            if income_data and len(income_data) > 0:
                ...
                logger.info(f"  - Revenue: ${latest_income.get('revenue', 0):,.0f}")
                logger.info(f"  - Net Income: ${latest_income.get('netIncome', 0):,.0f}")
                logger.info(f"  - EPS: {latest_income.get('eps', 0)}")
            
            if balance_data and len(balance_data) > 0:
                latest_balance = balance_data[0]
                logger.info(f"✅ Balance sheet:")
                logger.info(f"  - Total Assets: ${latest_balance.get('totalAssets', 0):,.0f}")
                logger.info(f"  - Total Debt: ${latest_balance.get('totalDebt', 0):,.0f}")
                logger.info(f"  - Total Equity: ${latest_balance.get('totalEquity', 0):,.0f}")
            
            if cashflow_data and len(cashflow_data) > 0:
                latest_cf = cashflow_data[0]
                logger.info(f"✅ Cash flow:")
                logger.info(f"  - Operating CF: ${latest_cf.get('operatingCashFlow', 0):,.0f}")
                logger.info(f"  - Free CF: ${latest_cf.get('freeCashFlow', 0):,.0f}")
                logger.info(f"  - CapEx: ${latest_cf.get('capitalExpenditure', 0):,.0f}")
            
            # 3. GET FINANCIAL RATIOS
            logger.info(f"[3/7] Fetching financial ratios...")
            progress.update_status("warren_buffett_agent", ticker, "Analyzing ratios")
            
            ratios = await self.tool_call_service.get_financial_ratios(
                symbol=ticker,
                period="annual",
                limit=5
            )
            ...
            logger.info(f"[4/7] Fetching growth metrics...")
            progress.update_status("warren_buffett_agent", ticker, "Analyzing growth")
            ...
            # 5. BUFFETT ANALYSIS
            logger.info(f"[5/7] Running Buffett analysis...")
            progress.update_status("warren_buffett_agent", ticker, "Calculating scores")
            
            # Analyze fundamentals using FMP data
            fundamental_score = self._analyze_fmp_fundamentals(
                key_metrics, 
                ratios[0] if ratios else None
            )
            
            # Analyze consistency
            consistency_score = self._analyze_fmp_consistency(
                income_data, 
                growth_data
            )
            
            # Analyze moat
            moat_score = self._analyze_fmp_moat(
                key_metrics,
                ratios
            )
            
            # Analyze management
            mgmt_score = self._analyze_fmp_management(
                balance_data,
                cashflow_data
            )
            
            # 6. CALCULATE INTRINSIC VALUE
            logger.info(f"[6/7] Calculating intrinsic value...")
            progress.update_status("warren_buffett_agent", ticker, "Valuation")
            
            intrinsic_value = self._calculate_fmp_intrinsic_value(
                cashflow_data,
                income_data
            )
            ...
            logger.info(f"[7/7] Calculating margin of safety...")
            
            margin_of_safety = None
            if intrinsic_value and market_cap and market_cap > 0:
                margin_of_safety = ((intrinsic_value - market_cap) / market_cap) * 100
                logger.info(f"  Margin of Safety: {margin_of_safety:.1f}%")
            
            # Generate signal
            total_score = fundamental_score + consistency_score + moat_score + mgmt_score
            max_score = 10 + 5 + 5 + 2  # fundamental + consistency + moat + management
            
            # Buffett-style signal generation
            if (total_score >= 0.7 * max_score) and margin_of_safety and (margin_of_safety >= 30):
                signal = "STRONG BUY"
            elif (total_score >= 0.6 * max_score) and margin_of_safety and (margin_of_safety >= 15):
                signal = "BUY"
            elif total_score >= 0.5 * max_score or (margin_of_safety and margin_of_safety > 0):
                signal = "HOLD"
            elif margin_of_safety and margin_of_safety < -30:
                signal = "SELL"
            else:
                signal = "NEUTRAL"
            
            logger.info(f"\n✅ {ticker} Analysis Complete!")
            logger.info(f"Signal: {signal}, Score: {total_score}/{max_score}, MOS: {margin_of_safety:.1f}%" if margin_of_safety else f"Signal: {signal}")
            ...
            return {
                ...
                "metrics": {
                    "pe_ratio": key_metrics.peRatioTTM if key_metrics else None,
                    "roe": key_metrics.roeTTM if key_metrics else None,
                    "debt_to_equity": key_metrics.debtToEquityTTM if key_metrics else None,
                    "current_ratio": key_metrics.currentRatioTTM if key_metrics else None,
                    "profit_margin": key_metrics.netIncomePerShareTTM / key_metrics.revenuePerShareTTM if key_metrics and key_metrics.revenuePerShareTTM else None,
                    "revenue": income_data[0].get('revenue') if income_data else None,
                    "net_income": income_data[0].get('netIncome') if income_data else None,
                    "free_cash_flow": cashflow_data[0].get('freeCashFlow') if cashflow_data else None
                }
            }
            
        except Exception as e:
            logger.error(f"Failed analyzing {ticker}: {str(e)}", exc_info=True)
            raise
    
    def _analyze_fmp_fundamentals(self, key_metrics, ratios) -> int:
        """Analyze fundamentals using FMP data matching Buffett criteria"""
        score = 0
        
        if not key_metrics:
            return 0
        
        # ROE Check (3 points) - Buffett loves ROE > 15%
        if key_metrics.roeTTM:
            roe = key_metrics.roeTTM
            if roe > 0.20:
                score += 3
                logger.info(f"  ✅ Excellent ROE: {roe*100:.1f}%")
            elif roe > 0.15:
                score += 2
                logger.info(f"  ✅ Good ROE: {roe*100:.1f}%")
            elif roe > 0.10:
                score += 1
        
        # Debt/Equity (3 points) - Buffett prefers low debt
        if key_metrics.debtToEquityTTM is not None:
            de = key_metrics.debtToEquityTTM
            if de < 0.5:
                score += 3
                logger.info(f"  ✅ Conservative debt: D/E {de:.2f}")
            elif de < 1.0:
                score += 2
            elif de < 1.5:
                score += 1
        
        # Operating Margin (2 points) - High margins = pricing power
        if ratios and hasattr(ratios, 'operatingProfitMargin'):
            margin = ratios.operatingProfitMargin
            if margin > 0.20:
                score += 2
                logger.info(f"  ✅ Strong margins: {margin*100:.1f}%")
            elif margin > 0.15:
                score += 1
        
        # P/E Ratio (2 points) - Reasonable valuation
        if key_metrics.peRatioTTM:
            pe = key_metrics.peRatioTTM
            if 10 < pe < 20:
                score += 2
                logger.info(f"  ✅ Reasonable P/E: {pe:.1f}")
            elif 5 < pe < 30:
                score += 1
        
        return min(score, 10)
    
    def _analyze_fmp_consistency(self, income_data, growth_data) -> int:
        """Analyze earnings consistency - Buffett loves predictable earnings"""
        score = 0
        ...
        earnings = [stmt.get('netIncome', 0) for stmt in income_data]
        earnings = [e for e in earnings if e]  # Filter out None/0
        
        if len(earnings) >= 3:
            if all(e > 0 for e in earnings):
                score += 3
                logger.info(f"  ✅ Consistently profitable: {len(earnings)} years")
            
            # Check earnings growth
            if len(earnings) >= 2 and earnings[0] > earnings[-1]:
                growth_rate = ((earnings[0] / earnings[-1]) ** (1/len(earnings))) - 1
                if growth_rate > 0.10:
                    score += 2
                    logger.info(f"  ✅ Strong earnings growth: {growth_rate*100:.1f}% CAGR")
        
        return min(score, 5)
    
    def _analyze_fmp_moat(self, key_metrics, ratios) -> int:
        """Analyze competitive advantages - Buffett's famous 'moat'"""
        score = 0
        
        if not key_metrics:
            return 0
        
        # Sustained high ROE = competitive advantage
        if key_metrics.roeTTM and key_metrics.roeTTM > 0.15:
            score += 2
            logger.info(f"  ✅ High sustained ROE suggests moat")
        
        # High profit margins = pricing power
        if ratios and len(ratios) > 0:
            latest_ratio = ratios[0]
            if hasattr(latest_ratio, 'netProfitMargin') and latest_ratio.netProfitMargin > 0.15:
                score += 2
                logger.info(f"  ✅ High margins suggest pricing power")
        
        # Low debt + high returns = quality business
        if (key_metrics.debtToEquityTTM and key_metrics.debtToEquityTTM < 0.5 and
            key_metrics.roeTTM and key_metrics.roeTTM > 0.15):
            score += 1
            logger.info(f"  ✅ Quality business model")
        
        return min(score, 5)
    
    def _analyze_fmp_management(self, balance_data, cashflow_data) -> int:
        """Analyze management quality - Buffett values good stewardship"""
        score = 0
        
        # Check dividend payments
        if cashflow_data and len(cashflow_data) > 0:
            latest_cf = cashflow_data[0]
            if latest_cf.get('dividendsPaid') and latest_cf['dividendsPaid'] < 0:  # Negative = paid out
                score += 1
                logger.info(f"  ✅ Pays dividends")
        
        # Check share buybacks
        if cashflow_data and len(cashflow_data) > 0:
            latest_cf = cashflow_data[0]
            if latest_cf.get('commonStockRepurchased') and latest_cf['commonStockRepurchased'] < 0:  # Negative = buyback
                score += 1
                logger.info(f"  ✅ Buying back shares")
        
        return min(score, 2)
    
    def _calculate_fmp_intrinsic_value(self, cashflow_data, income_data) -> Optional[float]:
        """
        Calculate intrinsic value using DCF with owner earnings
        This is Buffett's preferred method
        """
        ...
        net_income = 0
        if income_data and len(income_data) > 0:
            net_income = income_data[0].get('netIncome', 0)
        
        depreciation = latest_cf.get('depreciationAndAmortization', 0)
        capex = abs(latest_cf.get('capitalExpenditure', 0))  # Make positive
        
        owner_earnings = net_income + depreciation - capex
        
        # Use free cash flow as fallback
        if owner_earnings <= 0:
            owner_earnings = latest_cf.get('freeCashFlow', 0)
        
        if not owner_earnings or owner_earnings <= 0:
            logger.warning(f"  Cannot calculate IV - negative/zero owner earnings")
            return None
        
        # DCF parameters (Buffett conservative approach)
        growth_rate = 0.05  # Conservative 5% growth
        discount_rate = 0.09  # 9% required return
        terminal_multiple = 12  # Conservative terminal multiple
        years = 10
        
        # Calculate present value of future cash flows
        intrinsic_value = 0
        for year in range(1, years + 1):
            future_cf = owner_earnings * (1 + growth_rate) ** year
            pv = future_cf / (1 + discount_rate) ** year
            intrinsic_value += pv
        
        # Add terminal value
        terminal_cf = owner_earnings * (1 + growth_rate) ** years
        terminal_value = terminal_cf * terminal_multiple
        pv_terminal = terminal_value / (1 + discount_rate) ** years
        intrinsic_value += pv_terminal
        
        logger.info(f"  Intrinsic Value: ${intrinsic_value:,.0f}")
        logger.info(f"  (Owner Earnings: ${owner_earnings:,.0f})")
        
        return intrinsic_value
    
    def _format_fmp_analysis(self, analysis_data: Dict[str, Any]) -> str:
        ...
        for ticker, data in analysis_data.items():
            text = f"\n{'='*60}"
            text += f"\n{ticker} - BUFFETT VALUE ANALYSIS (FMP DATA)"
            text += f"\n{'='*60}"
            
            # Investment Signal
            text += f"\n\n🎯 INVESTMENT SIGNAL: {data.get('signal', 'NEUTRAL')}"
            
            # Margin of Safety
            if data.get('margin_of_safety') is not None:
                mos = data['margin_of_safety']
                text += f"\n💰 Margin of Safety: {mos:.1f}%"
                if mos > 30:
                    text += " ✅ EXCELLENT - Strong Buy"
                elif mos > 0:
                    text += " ⚠️ MODERATE - Consider"
                else:
                    text += " ❌ OVERVALUED - Avoid"
            
            # Valuation
            if data.get('intrinsic_value') and data.get('market_cap'):
                text += f"\n\n💎 VALUATION:"
                text += f"\n  Intrinsic Value: ${data['intrinsic_value']:,.0f}"
            ...
            if data.get('metrics'):
                ...
                text += f"\n\n📈 KEY METRICS:"
                if m.get('pe_ratio'):
                    text += f"\n  P/E Ratio: {m['pe_ratio']:.1f}"
                if m.get('roe'):
                    text += f"\n  ROE: {m['roe']*100:.1f}%"
                if m.get('debt_to_equity') is not None:
                    text += f"\n  Debt/Equity: {m['debt_to_equity']:.2f}"
                if m.get('profit_margin'):
                    text += f"\n  Net Margin: {m['profit_margin']*100:.1f}%"
                if m.get('revenue'):
                    text += f"\n  Revenue: ${m['revenue']:,.0f}"
                if m.get('net_income'):
                    text += f"\n  Net Income: ${m['net_income']:,.0f}"
                if m.get('free_cash_flow'):
                    text += f"\n  Free Cash Flow: ${m['free_cash_flow']:,.0f}"
            
            # Quality Scores
            if data.get('scores'):
                s = data['scores']
                text += f"\n\n🏆 BUFFETT QUALITY SCORES:"
                text += f"\n  Fundamentals: {s['fundamental']}/10"
                text += f"\n  Consistency: {s['consistency']}/5"
                text += f"\n  Economic Moat: {s['moat']}/5"
                text += f"\n  Management: {s['management']}/2"
                text += f"\n  TOTAL: {s['total']}/{s['max']} ({s['total']/s['max']*100:.0f}%)"
            
            output.append(text)
        
        return "\n".join(output)
    
    async def chat_general(self, message, context, personality, model_name, provider_type):
        """General chat without analysis"""
        
        system_prompt = f"""{personality}

Share investment wisdom as Warren Buffett."""
    ...
    def extract_tickers(self, message: str) -> List[str]:
        ...
        potential_tickers = re.findall(pattern, message)
        
        common_words = {
            "I", "A", "THE", "AND", "OR", "NOT", "BE", "TO", "OF", "IN",
            "FOR", "ON", "AT", "BY", "UP", "SO", "IT", "NO", "WE", "HE",
            "SHE", "YOU", "DO", "AN", "MY", "ONE", "ALL", "HAS", "HAD",
            "CAN", "ARE", "IS", "WAS", "IF", "BUT", "AS", "WITH", "THIS",
            "THAT", "HAVE", "FROM", "WILL", "WHAT", "WHEN", "WHERE", "HOW"
        }
