...
class Cache:
    ...
    def get_market_cap(self, key: str) -> Optional[float]:
        """
        Get cached market cap by composed key.
        Ex: key = f"market_cap:{ticker}:{end_date}"
        """
        return self._market_cap_cache.get(key)

    def set_market_cap(self, key: str, value: float) -> None:
        """
        Set cached market cap by composed key.
        """
