...
class StreamingMultiAgentService:
    ...
    async def stream_analysis(
        ...
        try:
            ...
            astream_gen = app.astream(initial_state, stream_mode="updates")
            ...
            try:
                while True:
                    ...
                    if pending_task is None:
                        pending_task = asyncio.create_task(anext(astream_gen))
                    
                    # Wait with timeout for heartbeat
                    done, _ = await asyncio.wait({pending_task}, timeout=DEFAULT_HEARTBEAT_SEC)
                    ...
                    if done:
                        ...
                        # Process chunk - GIỮ NGUYÊN LOGIC XỬ LÝ
                        for node_name, state_update in chunk.items():
                                    ...
                                    if agent_node_name in state_update["data"]["analyst_signals"]:
                                        ...
                                        for ticker, signal_data in agent_signals.items():
                                            if isinstance(signal_data, dict):
                                                processed_signals[ticker] = {
                                                    "signal": signal_data.get("signal", "neutral"),
                                                    "confidence": int(signal_data.get("confidence", 0.0)),
                                                    "reasoning": signal_data.get("reasoning", ""),
                                                    "metrics": signal_data.get("metrics", None),
                                                    "strategy_signals": signal_data.get("strategy_signals", None)
                                                }
